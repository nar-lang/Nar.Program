...
void state_free(nar_runtime_t rt) {
    ...
    if (state != NULL) {
        ...
        nar->free(state->subs);
        nar->free(state);
        nar->set_metadata(rt, NAR_METADATA_NAR_PROGRAM_STATE, NULL);
    }
}
...
void process_sub(nar_runtime_t rt, state_t *state, nar_object_t sub_object) {
    ...
    sub_state->user_data = sub->user_data;
    sub->on(rt, sub->user_data, trigger_sub, sub_state);
...
void resolve_command(nar_runtime_t rt, nar_object_t result, nar_cmd_state_t *cmd_state) {
    nar_object_t to_msg_list = nar->deserialize_object(rt, cmd_state->to_msg_list);
    enqueue_side_effect(rt, result, to_msg_list);
    nar->free(cmd_state->to_msg_list);
}
...
void process_command(nar_runtime_t rt, nar_serialized_object_t serialized_cmd) {
    ...
    cmd_t *cmd = nar->to_native(rt, cmd_object).ptr;
    nar_cmd_state_t *state = nar->alloc(sizeof(nar_cmd_state_t));
    state->to_msg_list = nar->new_serialized_object(rt, cmd->to_msg_list);
    nar->free(cmd);
    cmd->exec(rt, cmd->user_data, resolve_command, state);
}
...
void execute(nar_runtime_t rt, nar_object_t program, nar_program_exit_cb_t exit) {
    ...
    nar->set_metadata(rt, NAR_METADATA_NAR_PROGRAM_STATE, state);
