...
nar_object_t native__Cmd_map(nar_runtime_t rt, nar_object_t f, nar_object_t cmds) {
    ...
    while (nar->index_is_valid(rt, it)) {
        ...
        nar_object_t mapped_cmd = nar->make_tuple(rt, 3, (nar_object_t[]) {
                nar->make_list_cons(rt, f, cmd.values[0]), cmd.values[1], cmd.values[2]});
        vector_push(v, 1, &mapped_cmd);
        it = item.next;
    }
